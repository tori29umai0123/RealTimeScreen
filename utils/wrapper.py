        ...
        try:
            ...
            if acceleration == "tensorrt":
                ...
                def create_prefix(
                    ...
                    if lora_dict is not None:
                        for lora_name, lora_scale in lora_dict.items():
                            # ファイルのフルパスからファイル名（拡張子付き）を取得
                            base_name = os.path.basename(lora_name)
                            # ファイル名から拡張子を取り除く
                            file_name_without_extension = os.path.splitext(base_name)[0]
                            lora_name_scale = file_name_without_extension + "_" + str(lora_scale)

                    else:
                        lora_name_scale = None

                    if maybe_path.exists():
                        return f"{maybe_path.stem}--lcm_lora-{use_lcm_lora}--{lora_name_scale}--tiny_vae-{use_tiny_vae}--max_batch-{max_batch_size}--min_batch-{min_batch_size}--mode-{self.mode}"
                    else:
                        return f"{model_id_or_path}--lcm_lora-{use_lcm_lora}--{lora_name_scale}--tiny_vae-{use_tiny_vae}--max_batch-{max_batch_size}--min_batch-{min_batch_size}--mode-{self.mode}"
